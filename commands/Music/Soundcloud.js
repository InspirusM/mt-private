...
module.exports = class extends Command {
	...
	async run(msg, args, data) {
        ...
        if(this.client.utils.validateSoundcloud(query)) {
          let q = await soundcloud.getSongInfo(query);
          
          if(q.title.length > 40) q.title = q.title.slice(0, q.title.length - 32);
          
          query = q.title;
          
        }   
        
        const search = await soundcloud.search(query);
        ...
        collector.on('collect', async m => {
            ...
            const res = await this.client.player.search(track.url, msg.author.id);
